...
from comix_dl.core.cli import download_progress, flow_prompts
...
def test_render_series_info_panel_includes_truncated_metadata() -> None:
    ...
    with flow_prompts.console.capture() as capture:
        flow_prompts.render_series_info_panel(info)
...
def test_render_remote_api_error_outputs_message() -> None:
    with flow_prompts.console.capture() as capture:
        flow_prompts.render_remote_api_error(RemoteApiError("Cloudflare challenge still active"))
...
def test_prompt_chapter_selection_returns_none_for_quit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(flow_prompts, "filter_chapters_interactive", lambda items: items)
    monkeypatch.setattr(flow_prompts.Prompt, "ask", lambda *_args, **_kwargs: "q")

    assert flow_prompts.prompt_chapter_selection(chapters) is None
...
def test_prompt_chapter_selection_returns_empty_for_invalid_choice(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(flow_prompts, "filter_chapters_interactive", lambda items: items)
    monkeypatch.setattr(flow_prompts.Prompt, "ask", lambda *_args, **_kwargs: "7")
    monkeypatch.setattr(flow_prompts, "parse_chapter_selection", lambda *_args, **_kwargs: [])

    with flow_prompts.console.capture() as capture:
        result = flow_prompts.prompt_chapter_selection(chapters)
...
def test_prompt_chapter_selection_lists_selection_preview(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(flow_prompts, "filter_chapters_interactive", lambda items: items)
    monkeypatch.setattr(flow_prompts.Prompt, "ask", lambda *_args, **_kwargs: "all")
    monkeypatch.setattr(flow_prompts, "parse_chapter_selection", lambda *_args, **_kwargs: chapters)
...
def test_render_download_event_covers_all_supported_kinds() -> None:
    ...
    download_progress.render_download_event(
        progress,
        task_ids,
...
def test_render_download_summary_shows_issue_preview() -> None:
    ...
    with download_progress.console.capture() as capture:
        download_progress.render_download_summary(summary, Path("/tmp/output"))
